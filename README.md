...
- Переменные окружения:   
  ...
  - Volumes: Каждому брокеру предоставляется собственный том для хранения данных (```kafka_0_data```, ```kafka_1_data```, ```kafka_2_data```).  
...
- Скрипт [./scripts/create-topic.sh](./scripts/create-topic.sh) создает темы, монтируется как том, чтобы быть доступным внутри контейнера.  
